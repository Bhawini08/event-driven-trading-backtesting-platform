    ...
    def run(self):
        ...
        for i,(ts,row) in enumerate(self.prices.iterrows()):
            ...
            if can_trade and i % self.rebalance_every == 0 and i>0:
                # Signal uses information available through the prior close and
                # executes at the current bar, preventing same-close look-ahead.
                targets=self.strategy.targets(self.prices.iloc[:i])
                for order in self.portfolio.orders_from_targets(ts,targets,self.strategy.name,self.max_gross):
